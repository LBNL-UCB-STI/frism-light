...
parser.add_argument("-md", "--directory", dest="main_dir",
            help="where the population results are located ", required=True, type=str)
    ...
    for s in ship_type:
        for c in county_list:
            os.system("python frism_light_tour_update.py \
                        ...
                        -cy {4} \
                        -st {5} \
                        -t {6}/Geo_data/tt_df_cbg.csv.gz \
                        -d {6}/Geo_data/od_dist.csv\
                        -ct {6}/Geo_data/freight_centroids.geojson \
                        -vt {7}".format(args.main_sample_rate, args.main_dir, args.scenario, args.target_year, c, s,
                                        args.main_dir, veh_type_dir))
else:
